...
class Motor {
  ...
  public:
    ...
    uint8_t getIN1(); void setIN1(uint8_t in1);
    uint8_t getIN2(); void setIN2(uint8_t in2);
    uint8_t getEnable(); void setEnable(uint8_t enable);
    sentido_t getSentido(); void setSentido(sentido_t s);
...
void Motor::dancaMotor(float f_tensao, int tempo) { //Haha
  ...
  if (millis() - _last_time > tempo) {
    ...
    _sentido == HORARIO ? ANTI_HORARIO : HORARIO; // Altera o sentido de rotacao a cada vez que passar aqui
  }
}

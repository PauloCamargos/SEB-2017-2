...
/* DOCUMENTAÇÃO CLASSE MOTOR
 *  Classe desenvolvida por Paulo C. Silva, outubro/2017.
 *  Descrição: Classe para controle de motores utilizando o modulo l298n.
 *  Atributos: 
 *    _in1 - Pino IN1 conectado do módulo.
 *    _in2 - Pino IN2 conectado do módulo.
 *    _enable - Pino ENABLE do módulo.
 *    _sentido  - variável do tipo enum para armazenamento o sentido de rotação 
 *                do motor.
 *    _last_time - Variável auxiliar para função millis();
 *  Métodos: 
 *    getters() e setters();
 *    void rodaMotor(float f_tensao) - Inicia a rotação de acordo com a tensao f_tensao. 
 *      - Para f_tensao = 0, motor parado.
 *      - Para f_tensao >= 3.3/2, sentido horário. 
 *      - Para f_tensao <= 3.3/2, sentido anti-horário.
 *    void dancaMotor(int tempo) - Gira motor com sentido variado e intervalo definido (tempo).
 *    void giraHorario(float tensao) - Gira motor girar em sentido horário.
 *    void giraAntiHorario(float tensao) - Gira motor girar em sentido anti-horário.
 *    void stopMotor(float tensao) - Pára o motor.
 *    void desligaMotor() - Desativa (0) os pinos conectados ao módulo.
 */
...
class Motor {
  private:
    ...
    int _enable;
    sentido _sentido;
    int _last_time; //Variavel auxiliar para a função millis(), ignorar.
  public:
...
void Motor::stopMotor(float f_tensao) { // Freio magnético do motor
  setSentido(PARADO);
  digitalWrite(getIN1(), 1);
  digitalWrite(getIN2(), 1);

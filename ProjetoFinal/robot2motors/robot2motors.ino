...
void loop() {
  ...
  if (numbPackets >  0) { //Se tiver ao menos 1
    ...
    set_point = analogRead(ANALOG_SETPOINT) * 180.0 / 1024.0; //Le o SetPoint que pode ser de 0 a 360

    //----------------------------------------------------
    //Calculo do erro, deltaErro e DeltaT
    erro_angulo = set_point - angulo_lido;
    ...
    res_pid = res_pid < 0 ? 0 : res_pid; //Limite minimo de -255
    ...
    Serial.println(
      " SP: " + String(set_point, 2) + "\t" +
      " PID: " + String(res_pid, 2) + "\t" +
      " ANG: " + String(angulo_lido, 2) + "\t" +
      " Err: " + String(erro_angulo, 2)  + "\t" +
      " Kp: " + String(Kp * 10, 2)  + "\t" + //multipliquei por 100 para ficar mais facil de visualizar no serial Plotter
      " Ki: " + String(Ki * 100, 2)  + "\t" +
      " Kd: " + String(Kd * 100, 2)
    );
  }
  ...
  if (state_K == 0) {
    Kp = analogRead(ANALOG_K) * 10.0 / 1024.0; //Kp,Ki e Kd podem valer de 0 a 3
  } else if (state_K == 1) {
    Ki = 0;//analogRead(ANALOG_K) * 3.0 / 1024.0;
  } else if (state_K == 2) {
    Kd = 0;//analogRead(ANALOG_K) * 3.0 / 1024.0;
  }
}
...
// --- DEBOUCING
bool changeK() { // Debounce do botao liga desliga. Retorna 1 para ligado, 0 para desligado.
  // Ver referencia em https://www.arduino.cc/en/Tutorial/Debounce para entendimento da funcao
  int chReading = !digitalRead(PINO_K);
  ...
  if ((millis() - lastDeboucingTime) > debounceDelay) {
    if (chReading != chButtonState) {
      ...
      if (chButtonState) {
        estadoSistema = 1;
        //estadoSistema = !estadoSistema; //USAR EM ON/OFF
      }
    }
  }

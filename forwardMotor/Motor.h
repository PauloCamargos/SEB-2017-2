...
void Motor::stopMotor(float f_tensao) { // Freio magn�tico do motor
  ...
  digitalWrite(getIN1(), 1);
  digitalWrite(getIN2(), 1);
  analogWrite(getEnable(), 255);
  int t_mv = 1000*f_tensao;
  Serial.println("-- MOTOR PARADO | TENSAO: " + (String) f_tensao + " V");
}

void Motor::rodaMotor(float tensao) {
  giraHorario(tensao);
//  if (tensao == 3.3 / 2)
//    desligaMotor();
//  else
//    tensao > 3.3 / 2 ? giraHorario(tensao) : giraAntiHorario(tensao);
//  //Serial.println(" Tensao no ENCODER 1: " + String (analogRead(A1)) + "| Tensao no ENCODER 2: " + String (analogRead(A2)));
}
